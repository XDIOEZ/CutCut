...
整个文件夹复制进来即可安装，整个文件夹删除即可卸载。每个模块卡片都提供“管理配置”按钮；模块自带的设置页会在独立窗口中按需创建，窗口关闭或模块卸载后释放。新版本从下一次截图开始生效，已经打开的截图会安全地继续使用旧版本，直到该截图窗口关闭。
...
正式构建和完整包会自动把第一方截图收藏夹、贴图悬浮窗、长截图、本地 OCR 与二维码扫描模块分别放入
`Modules\Favorites`、`Modules\PinnedImage`、`Modules\LongCapture`、`Modules\Ocr` 和 `Modules\QrCode`，同时保持它们与主程序解耦，以便热替换和拆卸。
截图收藏夹在最终截图工具栏提供“收藏”按钮，并把最终图片按宿主当前 PNG/JPEG 格式直接写入模块配置窗口中选择的独立文件夹；该目录不跟随普通保存目录或日期分组。
...
分别识别后择优，不携带云端密钥或大型模型文件；本地 OCR 模块 1.3.0 需要轻截 1.11.7 或更高版本。
...
轻截 1.11.7 或更高版本，当前模块版本均为 1.2.0，并且必须完整保留入口 DLL、共享识别程序集、原生运行库、`Models` 与许可文件。
通常只启用本地、Tiny、Small 三者之一；如果同时安装，可在“插件模块”页独立启用、禁用或永久删除。
三种 OCR 都在本机识别图片；结果窗由宿主提供“翻译”按钮，只有用户主动点击时才把当前文字联网发送到
MyMemory 翻译为中文，截图图片和 OCR 模型不会上传。二维码结果仍使用不带翻译按钮的普通结果窗。
...
删除整个 `Modules\Favorites`、`Modules\PinnedImage`、`Modules\Ocr`、`Modules\PaddleOcrTiny`、`Modules\PaddleOcrSmall` 或 `Modules\QrCode`
