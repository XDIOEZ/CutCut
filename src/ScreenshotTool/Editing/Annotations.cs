...
internal sealed class PastedTextAnnotation : MovableAnnotation
{
    ...
    public override void Render(Graphics graphics, Bitmap source)
    {
        ...
        graphics.DrawRectangle(border, Bounds.X, Bounds.Y,
            Math.Max(1, Bounds.Width - 1), Math.Max(1, Bounds.Height - 1));
        var horizontalInset = Math.Min(8F, Math.Max(0F, (Bounds.Width - 1) / 2F));
        var verticalInset = Math.Min(6F, Math.Max(0F, (Bounds.Height - 1) / 2F));
        var textBounds = new RectangleF(
            Bounds.X + horizontalInset,
            Bounds.Y + verticalInset,
            Math.Max(1F, Bounds.Width - horizontalInset * 2),
            Math.Max(1F, Bounds.Height - verticalInset * 2));
