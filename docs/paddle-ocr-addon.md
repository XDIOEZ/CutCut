...
PP-OCR Tiny 与 PP-OCR Small 1.2.0 是两个彼此独立的可选模块。二者都完全在本机使用
PP-OCRv6 ONNX 模型识别截图选区，不上传图片，也不要求 Python；需要轻截 `1.11.7`
或更高版本。
...
识别图片和模型推理始终在本机完成。识别成功后的结果窗提供“翻译”按钮；只有用户主动点击时，
当前文字才会发送到 MyMemory 联网翻译为中文，截图图片不会上传。翻译后可用同一按钮在缓存的
原文和译文之间切换；恢复原文并编辑后再次点击，则会翻译最新文字。
