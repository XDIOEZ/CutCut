...
- “保存路径与命名”页提供“按日期自动创建子文件夹”开关，默认关闭以兼容旧配置。启用后显示独立的“日期分类父文件夹”路径框和浏览按钮，由用户手动绑定父目录；每次图片真正保存时按本地日期解析 `yyyy-MM-dd` 子目录，同一天复用已有目录，跨天自动创建新目录。关闭开关时继续使用原截图保存目录。该规则适用于普通截图、已保存图片再次编辑后的新图和模块通过宿主保存的图片，不改变录屏视频保存目录。相关入口：`Core/ScreenshotOutputFolderPolicy.cs`、`Infrastructure/PngImageSaveService.cs`、`Presentation/Pages/SavePathSettingsPage.cs`、`Presentation/MainForm.cs`。
...
- 图片贴纸被单独选中时，`Ctrl+C` 复制该图片当前缩放、旋转后的透明视觉结果且保持截图编辑窗口打开，便于继续粘贴或编辑；未单选图片时，`Ctrl+C` 仍复制整张截图选区并结束截图会话。工具栏“复制”和回车确认继续使用整张截图复制语义。
...
- 文字输入框聚焦时，`Ctrl+S` 仍属于截图级保存快捷键：先提交当前尚未结束的文字，再按现有规则保存 PNG、复制最终画面并关闭截图窗口；不得把 `S` 输入文字或遗漏当前文字内容。
