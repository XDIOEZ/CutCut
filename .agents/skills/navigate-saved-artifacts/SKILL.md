...
| 修改目标 | 首要入口 | 常见联动 |
| --- | --- | --- |
| 图片保存 | `ScreenshotTool/Abstractions/IImageSaveService.cs`、`Infrastructure/PngImageSaveService.cs` | 截图导出、剪贴板、通知 |
...
| 按日期解析截图子目录 | `ScreenshotTool/Core/ScreenshotOutputFolderPolicy.cs` | `PngImageSaveService.cs`、保存路径设置、历史画廊 |
...
- 改 `IModuleImageHost` 相关保存能力：同步检查模块契约与热卸载时的资源所有权。
