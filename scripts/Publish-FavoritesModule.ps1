...
$packageDirectory = [System.IO.Path]::GetFullPath(
    (Join-Path $outputRootPath "favorites-addon-win-x64"))
$buildDirectory = [System.IO.Path]::GetFullPath(
    (Join-Path $outputRootPath ".favorites-addon-build"))

foreach ($targetPath in @($packageDirectory, $buildDirectory)) {
    if (-not $targetPath.StartsWith(
            $outputRootPrefix,
            [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Favorites package paths must remain inside the selected output root: $targetPath"
    }
}
...
$moduleProject = Join-Path $repoRoot "src\ScreenshotTool.Favorites\ScreenshotTool.Favorites.csproj"
$moduleDirectory = Join-Path $packageDirectory "Modules\Favorites"

& dotnet publish $moduleProject -c Release -p:DebugSymbols=false -p:DebugType=None -o $buildDirectory
if ($LASTEXITCODE -ne 0) {
    throw "Favorites module publish failed with exit code $LASTEXITCODE."
}

$moduleSource = Join-Path $buildDirectory "ScreenshotTool.Favorites.dll"
if (-not (Test-Path -LiteralPath $moduleSource -PathType Leaf)) {
    throw "Favorites module was not published: $moduleSource"
}
...
Copy-Item -LiteralPath (Join-Path $repoRoot "docs\favorites-addon.md") `
    -Destination (Join-Path $packageDirectory "Favorites-README.md")
...
$modulePath = Join-Path $moduleDirectory "ScreenshotTool.Favorites.dll"
...
if ($packageBytes -gt 1MB) {
    throw "Favorites add-on size $([math]::Round($packageBytes / 1MB, 2)) MiB exceeds 1 MiB."
}

[pscustomobject]@{
    Package = "FavoritesAddon"
